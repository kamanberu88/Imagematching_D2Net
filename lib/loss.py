...
def loss_function_2D(
        model, batch, device, margin=1, safe_radius=1, scaling_steps=3, plot=False
):
    output = model({
    ...
    for idx_in_batch in range(batch['image1'].size(0)):
        ...
        fmap_pos2 = grid_positions(h2, w2, device)

        # Here, we simply use the same positions without warping, as it's 2D
        descriptors2 = F.normalize(
            dense_features2[:, fmap_pos2[0, :].long(), fmap_pos2[1, :].long()],
            dim=0
        )
